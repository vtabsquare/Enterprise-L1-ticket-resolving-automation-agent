...
def print_trace(ticket_id: str):
    ...
    # 2. Audit logs (chronological)
    resp = db.table("audit_logs").select("*").eq("ticket_id", ticket_id).order("timestamp", desc=False).execute()
    if not resp.data:
